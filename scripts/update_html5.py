...
<head>
    ...
    <style>
        ...
        .grid-container {{
            ...
            /* Responsive Static Scaling Logic */
            width: 100%;
            height: 100%;
            max-height: 100%;
            aspect-ratio: 23 / 25;
        ...
        @media (max-width: 1024px) {{
            .grid-container {{ max-height: 50vh; aspect-ratio: 23 / 25; margin: auto; }}
            .grid-cell {{ font-size: min(2vw, 10px); }}
        }}
    ...
    <div id="game-container" class="max-w-[1400px] mx-auto w-full flex-grow flex flex-col z-10 h-full">
        ...
        <div id="view-game" class="hidden-view main-workspace flex flex-col lg:flex-row gap-6 pointer-events-none opacity-50 transition-opacity duration-1000 w-full" style="filter: blur(5px);">
            ...
            <div class="lg:w-2/3 flex flex-col gap-4 h-full pointer-events-auto min-h-0">
                ...
                <!-- TOP: Crossword Grid Area (STATIC SCALED) -->
                <div class="cyber-card flex-grow p-2 md:p-4 rounded-xl flex items-center justify-center relative bg-black bg-opacity-50 border-opacity-60 overflow-hidden">
                    <div id="grid-root" class="grid-container m-auto"></div>
                </div>
            ...
            for(let i=0; i<GRID_H*GRID_W; i++) {{
                ...
                if(overlappingQuestions.length > 0) {{
                    ...
                    html = `<div class="grid-cell ${{classes}}" id="cell-${{x}}-${{y}}" onclick="selectQuestion('${{mainQ.id}}')">
                        ${{startMatch ? `<span style="position:absolute; top:0; left:1px; line-height:1; font-size:clamp(5px, 0.5vw, 8px); color:#06b6d4;">${{startMatch.display_id}}</span>` : ''}}
                        <span>${{letter}}</span>
                    </div>`;
                }} else {{
                    html += `</div>`;
...
# Copy to the cyber_mission_game folder as well to ensure synchronicity if user operates there
