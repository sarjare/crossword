...
<head>
    ...
    <style>
        ...
        .scanline {{ 
            ...
            display: none; /* Only activated on submit */
        }}
...
<body class="p-4 md:p-8 flex flex-col relative">
    ...
    <div id="game-container" class="max-w-7xl mx-auto w-full flex-grow flex flex-col z-10">
        ...
        <header class="flex flex-col md:flex-row justify-between items-center mb-6 border-b border-cyan-900 pb-4">
            ...
            <div class="flex gap-3 items-center mt-4 md:mt-0 flex-wrap justify-center">
                <button onclick="surrenderGame()" id="surrender-btn" class="hidden-view text-[10px] border border-red-900 px-3 py-1 hover:bg-red-900 hover:text-white transition-all font-bold orbitron text-red-500">SUBMIT EARLY</button>
                <button onclick="openLeaderboard()" id="leaderboard-btn" class="hidden-view text-[10px] border border-blue-900 px-3 py-1 hover:bg-blue-900 hover:text-white transition-all font-bold orbitron text-blue-400">LEADERBOARD</button>
                <button onclick="goAdminLogin()" class="text-[10px] border border-cyan-900 px-3 py-1 hover:bg-cyan-900 hover:text-cyan-400 transition-all font-bold orbitron text-cyan-700">OVERSEER</button>
            </div>
        </header>
        ...
        <div id="view-game" class="hidden-view flex-grow flex flex-col pointer-events-none opacity-50 transition-opacity duration-1000" style="filter: blur(5px);">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-grow min-h-0">
                ...
                <!-- Left Panel: Active Tool ONLY -->
                <div class="lg:col-span-3 h-full pointer-events-auto">
                    <div class="cyber-card p-5 h-full flex flex-col rounded-xl relative overflow-hidden">
                        <div class="absolute top-0 right-0 p-2 opacity-10 pointer-events-none"><i data-lucide="crosshair" class="w-24 h-24"></i></div>
                        <h3 class="orbitron text-cyan-400 mb-4 border-b border-cyan-900 pb-3 flex justify-between uppercase">
                            <span>DECRYPT NODE</span>
                            <span id="active-attempts" class="text-[10px] font-bold text-yellow-500 pt-1"></span>
                        </h3>
                        
                        <div id="active-q-container" class="hidden flex-grow flex flex-col">
                            <p class="text-[11px] font-bold text-cyan-500 mb-2 tracking-widest bg-cyan-900 bg-opacity-30 inline-block px-2 py-0.5 rounded" id="active-q-type"></p>
                            <p class="text-[15px] text-cyan-100 mb-6 flex-grow" id="active-q-hint"></p>
                            
                            <!-- Python Code Hint Area -->
                            <div id="active-q-python" class="hidden mb-4 p-3 bg-[#020617] border border-cyan-700 rounded text-[11px] font-mono text-cyan-300 overflow-x-auto whitespace-pre"></div>
                            
                            <div>
                                <input type="text" id="ans-input" class="terminal-input w-full p-3 text-xl font-bold uppercase tracking-widest mb-4 bg-black bg-opacity-30 rounded border border-cyan-900" placeholder="KEY...">
                                
                                <div class="flex gap-2">
                                    <button onclick="submitAnswer()" class="flex-grow py-4 bg-cyan-900 hover:bg-cyan-700 text-cyan-400 hover:text-white orbitron font-bold border border-cyan-500 transition-all text-sm rounded tracking-widest shadow-[0_0_10px_rgba(6,182,212,0.2)]">CRACK</button>
                                    <button id="hint-btn" onclick="takeHint()" class="hidden py-4 px-4 bg-yellow-900 bg-opacity-50 hover:bg-yellow-800 text-yellow-500 hover:text-yellow-300 orbitron font-bold border border-yellow-700 transition-all text-sm rounded tracking-widest" title="Get Hint (-10 Points)">HINT</button>
                                </div>
                                <p id="ans-feedback" class="text-[10px] mt-2 text-center h-4 text-red-500 font-bold uppercase tracking-widest"></p>
                            </div>
                        </div>
                        <div id="no-q-container" class="text-center py-20 text-cyan-900 opacity-70 flex-grow font-bold tracking-widest">
                            <i data-lucide="lock" class="w-16 h-16 mx-auto mb-4"></i>
                            <p class="text-xs uppercase mt-4">Select a node to begin decryption.</p>
                        </div>
                    </div>
                </div>

                <!-- Middle Panel: Crossword Grid -->
                <div class="lg:col-span-5 flex flex-col items-center justify-start pointer-events-auto h-full min-h-0 bg-black bg-opacity-40 p-4 rounded-xl border border-cyan-900 border-opacity-50 overflow-auto custom-scrollbar">
                    <div id="grid-root" class="grid-container w-full h-auto"></div>
                </div>

                <!-- Right Panel: Clues List (Side by Side) -->
                <div class="lg:col-span-4 cyber-card flex flex-col pointer-events-auto overflow-hidden rounded-xl min-h-0">
        ...
        <div id="view-leaderboard" class="hidden-view flex flex-col items-center justify-start w-full h-full max-w-4xl mx-auto space-y-6">
            ...
            <div class="cyber-card w-full p-4 rounded-xl flex-grow overflow-hidden flex flex-col">
                <div class="overflow-y-auto flex-grow text-sm space-y-2 pr-2 custom-scrollbar" id="full-leaderboard-list">
        ...
        <div id="view-admin-login" class="hidden-view flex flex-col items-center justify-center flex-grow space-y-8 animate-in fade-in">
            <div class="cyber-card p-10 w-full max-w-md space-y-6 rounded-xl border-yellow-600 text-center">
                ...
                <div class="flex gap-4">
                    <button onclick="closeAdminLogin()" class="flex-1 py-3 border border-gray-600 text-gray-400 orbitron border-opacity-50 hover:bg-gray-800">CANCEL</button>
                    <button onclick="submitAdminLogin()" class="flex-1 py-3 bg-yellow-700 text-black orbitron font-bold hover:bg-yellow-500 transition-all font-bold">ACCESS</button>
                </div>
    ...
    <script>
        ...
        const populateCluesList = () => {{
            ...
            const renderList = (arr, containerId) => {{
                document.getElementById(containerId).innerHTML = arr.map(q => `
                    <div id="clue-${{q.id}}" class="clue-list-item p-2 rounded mb-1 flex items-start gap-2 leading-snug" onclick="selectQuestion('${{q.id}}')">
        ...
        const updateCluesUI = () => {{
            ...
            QUESTIONS.forEach(q => {{
                ...
                if(team.solved.includes(q.id)) {{
                    ...
                    // auto scroll only if it's deeply out of view (smooth scroll sometimes jarring)
        ...
        window.submitAnswer = () => {{
            ...
            // Show scanline animation only during submit check
            ...
            if(inp === ans) {{
                ...
                setTimeout(() => {{
                    scan.style.display = 'none'; // hide scanline
                    state.activeQ = null;
