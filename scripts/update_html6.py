...
<head>
    ...
    <style>
        ...
        @media (max-width: 1024px) {{
            ...
            .grid-cell {{ width: min(4vw, 20px); height: min(4vw, 20px); font-size: 10px; border-width: 1px; }}
        }}
    ...
    <script>
        ...
        const setupDraggable = () => {{
            ...
            headerEl.addEventListener('mousedown', (e) => {{
                ...
                // If it still has the percentage transform, lock it to pixels for dragging
        ...
        window.selectQuestion = (qid) => {{
            ...
            document.getElementById('decrypt-prompt').classList.remove('hidden');
            
            // Re-center on open just to ensure it's visible if resolving off-screen errors
            const promptEl = document.getElementById('decrypt-prompt');
